...
#   - Lambda memory 512, log retention 30
...
module "vpc" {
  ...
  flow_log_retention_days = 30 # Staging
...
module "cognito" {
  ...
  name                   = "${var.project}-${var.environment}"
  environment            = var.environment
  mfa_configuration      = "ON" # Staging — bắt buộc MFA
...
module "dynamodb_users" {
  ...
  enable_pitr                 = true
  deletion_protection_enabled = true # Staging
...
module "sqs_compression" {
  ...
  visibility_timeout   = 720 # 12 phút (Lambda 60s × 12)
  max_receive_count    = 3
